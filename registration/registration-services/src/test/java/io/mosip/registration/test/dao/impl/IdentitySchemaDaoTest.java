...
public class IdentitySchemaDaoTest {
	...
	public void testSuccessSchemaSync() throws IOException {
		UiSchemaDTO dto = getSchemaDto("response_1587846312621.json");
	...
	private UiSchemaDTO getSchemaDto(String fileName) {
		UiSchemaDTO uiSchemaDTO = null;

		try {
			uiSchemaDTO = mapper.readValue(new File(getClass().getClassLoader().getResource(fileName).getFile()),
					UiSchemaDTO.class);
